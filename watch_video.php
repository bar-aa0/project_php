...
if(isset($_POST['delete_comment'])){
   ...
   if($verify_comment->rowCount() > 0){
      ...
      $message[] = 'نجح حذف التعليق!';
   }else{
      $message[] = 'تعليق محذوف بالفعل!';
   }
...
if(isset($_POST['update_now'])){
   ...
   if($verify_comment->rowCount() > 0){
      $message[] = 'تعليق مضاف بالفعل!';
   }else{
      $update_comment = $conn->prepare("UPDATE `comments` SET comment = ? WHERE id = ?");
      $update_comment->execute([$update_box, $update_id]);
      $message[] = 'تم تعديل تعليق!';
   }
...
<section class="edit-comment">
   <h1 class="heading">تعديل تعليق</h1>
   <form action="" method="post">
      <input type="hidden" name="update_id" value="<?= $fetch_edit_comment['id']; ?>">
      <textarea name="update_box" class="box" maxlength="1000" required placeholder="ادخل تعليقك" cols="30" rows="10"><?= $fetch_edit_comment['comment']; ?></textarea>
      <div class="flex">
         <a href="watch_video.php?get_id=<?= $get_id; ?>" class="inline-option-btn">الغاء تعديل</a>
...
<section class="watch-video">
   ...
   <div class="video-details">
      ...
      <form action="" method="post" class="flex">
         ...
         <a href="playlist.php?get_id=<?= $fetch_content['playlist_id']; ?>" class="inline-btn">مشاهدة القائمة</a>
         <?php
            if($verify_likes->rowCount() > 0){
         ?>
         <button type="submit" name="like_content"><i class="fas fa-heart"></i><span>اعجب</span></button>
         <?php
         }else{
         ?>
         <button type="submit" name="like_content"><i class="far fa-heart"></i><span>إعجاب</span></button>
         <?php
            }
         ?>
      </form>
   ...
   <?php
         ...
         echo '<p class="empty">لم يتم إضافة تعليق بعد!</p>';
      }
   ?>
...
<section class="comments">
   ...
   <h1 class="heading">إضافت تعليق</h1>

   <form action="" method="post" class="add-comment">
      <input type="hidden" name="content_id" value="<?= $get_id; ?>">
      <textarea name="comment_box" required placeholder="اكتب تعليق" maxlength="1000" cols="30" rows="10"></textarea>
      <input type="submit" value="إضافة تعليق" name="add_comment" class="inline-btn">
   </form>

   <h1 class="heading">تعليقات المستخدمين</h1>
   ...
   <div class="show-comments">
      ...
      <div class="box" style="<?php if($fetch_comment['user_id'] == $user_id){echo 'order:-1;';} ?>">
         ...
         <form action="" method="post" class="flex-btn">
            ...
            <button type="submit" name="delete_comment" class="inline-delete-btn" onclick="return confirm('هل تريد حذف تعليق?');">حذف التعليق</button>
         </form>
         <?php
         }
         ?>
      ...
      <?php
         ...
         echo '<p class="empty">لايوجد تعليقات!</p>';
      }
      ?>
      </div>
